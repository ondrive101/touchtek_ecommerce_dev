...
export default function OrdersPage() {
  ...
  if (isError || !salesOrders?.data) {
    const errorCode = ERROR_CODES.ORDERS_FETCH_FAILED;
    return (
  ...
  const filtered = orders.filter((o) => {
    ...
    const matchMonth =
      selectedMonth === "All Months" || o.month === selectedMonth;
      ...
      <div className="max-w-4xl mx-auto w-full px-4 sm:px-6 pt-12 pb-6">
        ...
        >
          ...
          <p className="text-sm text-gray-500">
            {orders.length} orders placed · Track, return or reorder
          </p>
        </motion.div>
      </div>
        ...
        <AnimatePresence mode="wait">
          {filtered.length === 0 ? (
            ...
            >
              ...
              <a
                href="/shop"
                className="inline-flex items-center gap-2 bg-gradient-to-r from-black to-gray-800 text-white px-6 py-3 rounded-xl font-bold text-sm shadow-lg"
              >
                    ...
                    >
                      <div className="flex items-start justify-between gap-3 flex-wrap">
                        ...
                        <div className="flex items-center gap-3 ml-auto">
                          ...
                          <span
                            className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border ${status.bg} ${status.color} ${status.border}`}
                          >
                            <span
                              className={`w-1.5 h-1.5 rounded-full ${status.dot} ${order.status === "processing" ? "animate-pulse" : ""}`}
                            />
                            {status.label}
                          </span>
                          <motion.div
                      ...
                      {!isExpanded && (
                        <div className="mt-3 flex items-center gap-2 flex-wrap">
                          {order?.items.map((item, idx) => (
                            <span
                              key={idx}
                              className="inline-flex items-center gap-1.5 bg-gray-50 border border-gray-100 px-2.5 py-1 rounded-lg text-xs text-gray-600 font-medium uppercase"
                            >
                              {item.name}
                          ...
                          <div className="border-t border-gray-100 px-5 pb-5 pt-4 space-y-5">
                            ...
                            {order.status === "shipped" && (
                              <div className="flex items-center justify-between gap-2 bg-blue-50 border border-blue-200 rounded-xl px-4 py-2.5 flex-wrap">
                                ...
                                <span className="text-xs font-mono text-blue-600 font-bold bg-blue-100 px-2 py-0.5 rounded-lg">
                                  {order.trackingId}
                                </span>
                              </div>
                            )}
                            ...
                            {order.status === "returned" && (
                              <div className="flex items-center justify-between gap-2 bg-purple-50 border border-purple-200 rounded-xl px-4 py-2.5 flex-wrap">
                                ...
                                <span className="text-xs font-bold text-green-700 bg-green-100 border border-green-200 px-2 py-0.5 rounded-lg">
                                  {order.refundStatus}
                                </span>
                              </div>
                            )}
                                      ...
                                      {order.status === "delivered" && (
                                        <div className="flex items-center gap-0.5 mt-1.5">
                                          {[1, 2, 3, 4, 5].map((star) => (
                                            ...
                                            >
                                              <Star
                                                className={`w-3.5 h-3.5 transition-colors ${star <= (ratingMap[`${order.id}-${idx}`] || 0) ? "fill-yellow-400 text-yellow-400" : "text-gray-300"}`}
                                              />
                                            </motion.button>
                                          ))}
                                          <span className="text-xs text-gray-400 ml-1">
                                            {ratingMap[`${order.id}-${idx}`]
                                              ? "Rated!"
                                              : "Rate product"}
                                          </span>
                                        </div>
                            ...
                            <div className="bg-gradient-to-br from-gray-50 to-white border border-gray-100 rounded-xl p-4 space-y-2">
                              ...
                              {order.rewardDiscount > 0 && (
                                <div className="flex justify-between text-xs text-yellow-600 font-semibold">
                                  <span className="flex items-center gap-1">
                                    <Coins className="w-3 h-3" /> Reward Points
                                  </span>
                                  <span>
                                    − ₹{order.rewardDiscount.toLocaleString()}
                                  </span>
                                </div>
                              )}
                              ...
                              {order.status === "delivered" && (
                                <>
                                  <Btn
                                    icon={RefreshCw}
                                    label="Reorder"
                                    className="bg-gradient-to-r from-black to-gray-800 text-white shadow-md"
                                  />
                                  <Btn
                                    icon={RotateCcw}
                                    label="Return"
