...
export default function ProductPage({searchParams}) {
  ...
  const [relatedProducts, setRelatedProducts] = useState([]);
    ...
    const { data:productData, isLoading, isError, error, isFetching } = useQuery({
     ...
     useEffect(() => {
          if (productData) {
            console.log("productsData received",productData);
...
<div className="bg-white rounded-2xl p-6 shadow-lg">
  ...
  <ul className="space-y-3">
    {[
      // "Battery Type: Lithium – Original Capacity",
      // "Fast Charging: Charges faster than standard batteries",
      // "Durability: Highly durable, reliable technology",
      // "Protection: Over-voltage, over-current, overcharge, and short-circuit protection",
      ...(variant?.attributes?.color ? [`Color: ${variant?.attributes?.color}`] : []),
      ...(variant?.attributes?.port ? [`Port Type: ${variant?.attributes?.port}`] : [])
    ].map((spec, index) => (
      <motion.li
        key={index}
        ...
        <span className="text-gray-700">{spec}</span>
              ...
              <div className="bg-gradient-to-r from-gray-800 to-black rounded-2xl p-6 text-white">
                ...
                <p className="text-gray-200 mb-4">
                  Get in touch with our team for more information or bulk
                  orders.
                </p>
        ...
        <section className="bg-white py-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              ...
              <p className="text-gray-600">Discover more products in the {product?.subCategory?.name} category</p>
            </motion.div>
                      ...
                      <p className="text-sm text-gray-600 mb-3 line-clamp-2">
                       ...
                       {/* Rating */}
                                ...
                                <div className="flex items-baseline gap-2 mb-3">
                                  ...
                                  {relatedProduct?.originalPrice && relatedProduct?.price && relatedProduct?.originalPrice > relatedProduct?.price && (
