...
export default function Header() {
  ...
  const profileMenuItems = [
    { href: '/en/profile',       label: 'Profile',       icon: User2 },
    { href: '/en/notifications', label: 'Notifications', icon: Bell },
    { href: '/en/support',       label: 'Support',       icon: Headphones },
    { href: '/en/orders',        label: 'Orders',        icon: Package },
    { href: '/en/rewards',       label: 'Rewards',       icon: Award },
    { href: '/en/affiliates',    label: 'Affiliates',    icon: Users },
  ];
        ...
        {isMenuOpen && (
          <div className="md:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-gray-50 border-t border-gray-100">
              <Link
                href="/en/products"
                className="block px-3 py-2 text-base font-medium text-gray-700 hover:text-black rounded-lg hover:bg-white transition-all"
              ...
              {isLoading ? (
                ...
                <>
                  <Link
                    href="/en/login"
                    className="group relative flex items-center justify-center gap-2 px-4 py-3 mt-2 mx-2 bg-gradient-to-r from-gray-800 to-black text-white text-base font-semibold rounded-2xl shadow-lg hover:shadow-2xl hover:from-gray-900 hover:to-black active:scale-[0.98] transition-all duration-200 overflow-hidden"
                    onClick={() => setIsMenuOpen(false)}
                  >
                ...
                <>
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      <ChevronDown className={`w-5 h-5 ml-auto transition-transform duration-200 ${isProfileOpen ? 'rotate-180' : ''}`} />
                    </button>

                    {isProfileOpen && (
                      <div className="mt-1 bg-white rounded-2xl shadow-2xl border border-gray-100 py-2 z-50">
                        {/* User info header */}
                        <div className="px-4 py-2 border-b border-gray-100 mb-1">
                          <p className="text-sm font-semibold text-gray-900">{session?.user?.name}</p>
                          <p className="text-xs text-gray-500 truncate">{session?.user?.email}</p>
                  ...
                  >
                    ...
                    <span>Cart</span>
                    <span className="ml-auto bg-red-500 text-white text-xs rounded-full w-6 h-6 flex items-center justify-center font-bold">{items?.length}</span>
                  </Link>
                </>
              )}
            </div>
